...
function draw() {
  ...
  if(frameCount%updatetime==0){
    
   // console.log("mouseX "+ corner);
   ...
   // console.log("d " + d); 
    ...
    if(d>=corner+1){
      inflate=0;
      pause++;
      if(pause>100)inflate=-0.1;//updatetime=50;
    }
  ...
  // d = dist(mouseX, mouseY, width / 2, height / 2);
   corner= dist(0,0,windowWidth/2, windowHeight/2);
  zoom = map(d, 0, corner, -1000, -10);
 number = map(d, 0, corner, 250, 30);
  translate(0, 0, zoom);
